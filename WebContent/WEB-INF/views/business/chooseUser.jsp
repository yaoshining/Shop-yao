		...
		callback: {
			onClick: function(event, treeId, treeNode, clickFlag){
				if(!treeNode.isParent){
					var url = '<s:url value="/biz/users"></s:url>/'+treeNode.id;
					$.ajax({
						url : url,
						type : "get",
						dataType : "json",
						beforeSend : function(){
							...
							$source.effect("transfer", {
								...
								$helper.remove();
								$.fn.zTree.init($("#userTree"), userTreeSetting,deptNode);
							} );
						},
						success : function(data) {
							setTimeout(function(){
								var userTree = $.fn.zTree.getZTreeObj("userTree");
								var node = userTree.getNodeByParam("id",treeNode.id,null);
								userTree.addNodes(node,data,true);
								userTree.expandNode(node, true, false, false);
							},1500);
						}
					});
				}
		...
		callback: {
			...
			onCheck : function(event,treeId,treeNode){
				if(treeNode.checked){
					...
					selectedUserIds.pop(treeNode.id);
				}
			}
		},
		check: {
...
function validate(){
	var userTree = $.fn.zTree.getZTreeObj("userTree");
	return userTree&&userTree.getCheckedNodes(true).length>0;
}
//-->
</script>
<style type="text/css">
